...
export default function Dashboard({ token, user, onLogout, onUserUpdate }: { token: string, user: any, onLogout: () => void, onUserUpdate: (u: any) => void }) {
  ...
  return (
    <div className={`theme-page flex flex-col min-h-screen items-stretch p-6 sm:p-12 font-sans overflow-y-auto no-scrollbar relative border-l ${isDark ? 'bg-[#09090B] border-white/10' : 'bg-white border-gray-200'}`}
      style={{
        paddingLeft: isSidebarMinimized ? '120px' : `${sidebarWidth + 60}px`,
        transition: isResizing ? 'none' : 'padding-left 0.3s ease',
    ...
    >
      ...
      <div
        ref={sidebarRef}
        className={`fixed top-6 bottom-6 z-50 flex flex-col rounded-2xl border shadow-2xl overflow-hidden transition-all ${
          isSidebarMinimized ? 'left-4 w-20' : 'left-4'
        } ${
          isDark ? 'bg-[#0f1117]/90 border-white/10' : 'bg-white/90 border-gray-200'
        }`}
        style={{
          width: isSidebarMinimized ? '80px' : `${sidebarWidth}px`,
          transitionDuration: isResizing ? '0ms' : '300ms'
        }}
      >
        {/* Minimize Toggle Button at Top */}
        <div className="p-3 border-b flex items-center justify-center" style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}>
          <button
            type="button"
            onClick={() => setIsSidebarMinimized(!isSidebarMinimized)}
            className={`flex h-10 w-10 items-center justify-center rounded-xl border transition-all ${
              isDark
                ? 'bg-[#1A1A1E] hover:bg-[#25252D] border-white/10 text-gray-300'
        ...
        {!isSidebarMinimized && (
          <div className="px-2 py-2 flex flex-col gap-1">
            ...
            <div className="flex gap-1">
              {/* Page Users Card */}
              <div className={`flex-1 rounded-lg p-2 border ${isDark ? 'bg-blue-500/10 border-blue-400/20' : 'bg-blue-50 border-blue-200'}`}>
                <p className={`text-[9px] font-bold uppercase tracking-wide mb-0.5 ${isDark ? 'text-blue-300/80' : 'text-blue-600'}`}>
                  Page Users
                </p>
                <div className="flex items-end justify-between mb-1">
                  <p className={`text-xl font-bold ${isDark ? 'text-blue-100' : 'text-blue-800'}`}>
                    {users.length}
                  </p>
                  <p className={`text-[11px] font-semibold ${isDark ? 'text-blue-300' : 'text-blue-700'}`}>
                    {totalUsersCount > 0 ? Math.round((users.length / totalUsersCount) * 100) : 0}%
                  </p>
                </div>
                <div className={`h-1 rounded-full overflow-hidden ${isDark ? 'bg-blue-950/50' : 'bg-blue-100'}`}>
                  <div
                    className={`h-full rounded-full ${isDark ? 'bg-blue-400' : 'bg-blue-500'}`}
                    style={{ width: `${totalUsersCount > 0 ? Math.round((users.length / totalUsersCount) * 100) : 0}%` }}
                  />
                </div>
              </div>

              {/* Overall Users Card */}
              <div className={`flex-1 rounded-lg p-2 border ${isDark ? 'bg-purple-500/10 border-purple-400/20' : 'bg-purple-50 border-purple-200'}`}>
                <p className={`text-[9px] font-bold uppercase tracking-wide mb-0.5 ${isDark ? 'text-purple-300/80' : 'text-purple-600'}`}>
                  Overall Users
                </p>
                <div className="flex items-end justify-between mb-1">
                  <p className={`text-xl font-bold ${isDark ? 'text-purple-100' : 'text-purple-800'}`}>
                    {totalUsersCount}
                  </p>
                  <p className={`text-[11px] font-semibold ${isDark ? 'text-purple-300' : 'text-purple-700'}`}>
                    100%
                  </p>
                </div>
                <div className={`h-1 rounded-full overflow-hidden ${isDark ? 'bg-purple-950/50' : 'bg-purple-100'}`}>
                  <div className={`h-full rounded-full ${isDark ? 'bg-purple-400' : 'bg-purple-500'}`} style={{ width: '100%' }} />
                </div>
              </div>
            </div>
          </div>
        ...
        {/* Bottom Section with Navigation Icons and User Profile */}
        {!isSidebarMinimized && (
          <>
            {/* Navigation Buttons */}
            <div className={`px-3 py-3 border-t border-b flex flex-col gap-2`} style={{ borderColor: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.1)' }}>
              <button
                type="button"
                onClick={() => switchListView('companies')}
              ...
              >
                <span className={`flex h-8 w-8 items-center justify-center rounded-lg border ${
                  isCompanyListView
                    ...
                    : isDark
                      ? 'border-white/10 bg-[#121214] text-gray-300'
                      : 'border-gray-200 bg-white text-gray-700'
                }`}>
                  <Building2 size={16} />
                </span>
                <span className="text-sm font-semibold">Company List</span>
              </button>

              <button
                type="button"
                onClick={() => switchListView('users')}
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl border transition-all ${
                  !isCompanyListView
                    ? isDark
            ...
            <div className={`p-4 flex flex-col gap-3`}>
              <div className="flex items-center gap-3">
                {headerAvatarUrl ? (
                  <img src={headerAvatarUrl} alt="Profile" className="h-10 w-10 rounded-full object-cover border" style={{ borderColor: isDark ? 'rgba(59,130,246,0.4)' : 'rgba(59,130,246,0.6)' }} />
                ) : (
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => openUserEditor(currentUserRecord)}
                  className={`flex-1 inline-flex items-center justify-center gap-1 px-2 py-2 rounded-lg text-xs font-semibold border transition-all ${
                    isDark
                      ? 'text-blue-300 bg-blue-500/10 border-blue-400/35 hover:bg-blue-500/18'
                      : 'text-blue-700 bg-blue-100 border-blue-300 hover:bg-blue-200'
                  }`}
              ...
              <button
                type="button"
                onClick={onLogout}
                className={`w-full flex items-center justify-center gap-2 px-3 py-2.5 rounded-lg text-sm font-semibold border transition-all ${
                  isDark
                    ? 'text-gray-300 bg-[#1A1A1E] hover:bg-[#25252D] border-white/10'
        ...
        {/* Minimize State - Show All Icons Horizontally */}
        {isSidebarMinimized && (
          <div className="w-full px-1 flex flex-row items-center justify-start gap-1.5 overflow-x-auto no-scrollbar">
            <button
              type="button"
              onClick={() => switchListView('companies')}
        ...
        {/* Profile Avatar in Minimized State */}
        {isSidebarMinimized && (
          <div className="flex flex-col items-center justify-center p-2">
            {headerAvatarUrl ? (
              <img 
                src={headerAvatarUrl} 
                alt="Profile" 
                className="h-10 w-10 rounded-full object-cover border cursor-pointer transition-all hover:opacity-80" 
                style={{ borderColor: isDark ? 'rgba(59,130,246,0.4)' : 'rgba(59,130,246,0.6)' }}
                title={userDisplayName}
              />
            ) : (
              <div 
                className={`h-10 w-10 rounded-full flex items-center justify-center font-semibold text-sm cursor-pointer transition-all hover:opacity-80 ${
                  isDark ? 'bg-blue-500/20 text-blue-300' : 'bg-blue-100 text-blue-700'
                }`}
                title={userDisplayName}
              >
